...
<body>
    ...
    <div class="container">
        ...
        class MPEmail extends RecursiveIteratorIterator {
        ...
        try {
            ...
            $searchTerm = $_POST['uemail'];
            // prepare statement for executions. This part needs to change for every query
            // Find the movies that have been liked by a specific user’s email (parameterized). List the
            // movie name, rating, production and budget.
            $stmt = $conn->prepare("SELECT mp.name, rating, production, budget FROM MotionPicture mp JOIN Movie m ON mp.id=m.mpid JOIN Likes l ON mp.id=l.mpid JOIN User u ON l.uemail=u.email WHERE u.email=:searchTerm;");
            ...
            if(isset($_POST['mpemail']))
            {
            echo "<h1>Liked movies</h1>";
            // we will now create a table from PHP side 
            echo "<table class='table table-md table-bordered'>";
            echo "<thead class='thead-dark' style='text-align: center'>";

            // initialize table headers
            echo "<tr><th class='col-md-2'>Movie name</th>
                ...
                foreach(new MPEmail(new RecursiveArrayIterator($stmt->fetchAll())) as $k=>$v) {
                    echo $v;
                }
            }
        }
